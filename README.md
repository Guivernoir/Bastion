...
- MLSigcrypt-v3 level-3 signcryption over a noisy algebraic encapsulation + ML-DSA-87
...
- MLSigcrypt-v3 level 3 uses a split packet layout (`encap | z | c_tilde | h | ct_len | ct`) with a noisy algebraic `u || v` encapsulation and an 8393-byte fixed overhead in the current exact-encoding implementation.
