...
use crate::algos::mlkem1024::field::{INV_NTT_SCALE, barrett_reduce, fqmul};
...
pub(crate) const ZETAS: [i16; 128] = [
    ...
    1223, 652, -552, 1015, -1293, 1491, -282, -1544, 516, -8, -320, -666, -1618, -162, 126, 1469,
...
pub(crate) fn poly_basemul(a: &[i16; 256], b: &[i16; 256], out: &mut [i16; 256]) {
    out.fill(0);
    for i in 0..128usize {
        // Kyber/FIPS ordering uses one base zeta per two adjacent pairs;
        // the odd pair uses the negated zeta.
        let base = ZETAS[64 + i / 2];
        let zeta = if (i & 1) == 0 { base } else { -base };
        let j = 2 * i;
        let a0 = a[j];
        let a1 = a[j + 1];
        let b0 = b[j];
        let b1 = b[j + 1];
        out[j] = fqmul(a0, b0).wrapping_add(fqmul(fqmul(a1, b1), zeta));
        out[j + 1] = fqmul(a0, b1).wrapping_add(fqmul(a1, b0));
    }
}
