use crate::algos::mlkem1024::hash::{shake128_xof_init, shake256_prf};
use crate::algos::mlkem1024::keccak::zeroize_sponge;
...
use crate::algos::mlkem1024::params::{ETA1, ETA1_PRF_BYTES, ETA2, ETA2_PRF_BYTES, K, N};
use crate::algos::mlkem1024::poly::Poly;
use crate::algos::mlkem1024::vec::PolyVec;
...
///
/// Process 4 coefficients per 16-bit word: fast path for η=2.
#[inline]
fn cbd2(p: &mut Poly, buf: &[u8]) {
    // Each byte yields 2 coefficients: bits [1:0] − bits [3:2] and [5:4] − [7:6].
    // Process as u16 to get 4 coefficients at a time.
    debug_assert_eq!(buf.len(), 128);
    for i in 0..64usize {
        let word = u16::from_le_bytes([buf[2 * i], buf[2 * i + 1]]) as u32;

        // Separate odd and even bits, then popcount pairs.
        // a[k] = popcount of bits {2k, 2k+1} in lower half
        // b[k] = popcount of bits {2k, 2k+1} in upper half
        // This is the standard Kyber CBD2 bit-manipulation trick.
        let d = word & 0x5555u32; // even bits
        let e = (word >> 1) & 0x5555u32; // odd bits
        let f = d.wrapping_add(e); // 2-bit popcounts per pair (η=2 → max 2)

        // Each nibble now holds (a, b) where a and b are in [0, 2].
        // coefficient = low 2 bits − high 2 bits of each nibble.
        // Since we're doing 4 coefficients per u32, extract each nibble pair.
        for j in 0..4usize {
            let shift = 4 * j;
            let a = ((f >> shift) & 0x3) as i16;
            let b = ((f >> (shift + 2)) & 0x3) as i16;
            p.coeffs[4 * i + j] = a - b;
        }
    }
}
...
fn parse_poly_from_xof(p: &mut Poly, rho: &[u8; 32], i: u8, j: u8) {
    ...
    'outer: loop {
        ...
        while pos + 3 <= buf.len() {
            ...
            if d1 < crate::algos::mlkem1024::params::Q as u16 {
            ...
            if d2 < crate::algos::mlkem1024::params::Q as u16 {
