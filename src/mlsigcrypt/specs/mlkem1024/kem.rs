...
use crate::mlsigcrypt::specs::mlkem1024::hash::{sha3_256, sha3_512_x2, shake256_x2};
...
fn keygen_from_rho_sigma(
    ...
    for p in t_hat.polys.iter_mut() {
        crate::mlsigcrypt::specs::mlkem1024::poly::poly_to_mont(p);
    }
    t_hat.add_reduce(&e);
...
fn kpke_enc(
    ...
    let mut t_hat = PolyVec::zero();
    {
        ...
        // ── Generate Aᵀ ───────────────────────────────────────────────────────
        let mut a_hat = PolyMatrix::zero();
        gen_matrix(&mut a_hat, &rho_local, true);
...
mod tests {
    ...
    #[test]
    fn kpke_enc_dec_recovers_message() {
        let seed = [0x42u8; KEYGEN_SEED_BYTES];
        let msg = [0xA5u8; 32];
        let r = [0x11u8; 32];
        ...
        let dk_pke: &[u8; DK_PKE_BYTES] =
            unsafe { &*(dk.0.as_ptr().add(DK_OFFSET_DK_PKE) as *const [u8; DK_PKE_BYTES]) };
        kpke_enc(&ek.0, &msg, &r, &mut ct);
        kpke_dec(dk_pke, &ct, &mut recovered);
    ...
    fn kem_encaps_decaps_agree() {
        let seed = [0x24u8; KEYGEN_SEED_BYTES];
        let entropy = [0x18u8; 32];
        let mut ek = EncapKey([0u8; EK_BYTES]);
        let mut dk = DecapKey([0u8; DK_BYTES]);
        let mut ct = Ciphertext([0u8; CT_BYTES]);
        let mut ss_a = SharedSecret([0u8; SS_BYTES]);
        let mut ss_b = SharedSecret([0u8; SS_BYTES]);

        keygen(&seed, &mut ek, &mut dk);
        encaps(&ek, &entropy, &mut ct, &mut ss_a);
        decaps(&dk, &ct, &mut ss_b);

        assert_eq!(ss_a.as_bytes(), ss_b.as_bytes());
    }

    #[test]
    fn key_material_encoding_roundtrips() {
        let seed = [0x77u8; KEYGEN_SEED_BYTES];
        let mut ek = EncapKey([0u8; EK_BYTES]);
        let mut dk = DecapKey([0u8; DK_BYTES]);
        let mut t_hat = PolyVec::zero();
        let mut rho = [0u8; 32];
        let mut s_hat = PolyVec::zero();
        let mut ek_roundtrip = [0u8; EK_BYTES];
        let mut dk_roundtrip = [0u8; DK_PKE_BYTES];

        keygen(&seed, &mut ek, &mut dk);

        decode_ek(&ek.0, &mut t_hat, &mut rho);
        encode_ek(&t_hat, &rho, &mut ek_roundtrip);

        let dk_pke: &[u8; DK_PKE_BYTES] =
            unsafe { &*(dk.0.as_ptr().add(DK_OFFSET_DK_PKE) as *const [u8; DK_PKE_BYTES]) };
        decode_dk_pke(dk_pke, &mut s_hat);
        encode_dk_pke(&s_hat, &mut dk_roundtrip);

        assert_eq!(ek_roundtrip, ek.0);
        assert_eq!(dk_roundtrip, *dk_pke);
    }

    #[test]
    fn raw_pke_arithmetic_roundtrip() {
        let seed = [0x33u8; KEYGEN_SEED_BYTES];
        let msg = [0xA5u8; 32];
        let r_seed = [0x11u8; 32];
        let mut ek = EncapKey([0u8; EK_BYTES]);
        let mut dk = DecapKey([0u8; DK_BYTES]);
        let mut t_hat = PolyVec::zero();
        let mut rho = [0u8; 32];
        let mut s_hat = PolyVec::zero();
        let mut at = PolyMatrix::zero();
        let mut r_vec = PolyVec::zero();
        let mut e1 = PolyVec::zero();
        let mut e2 = Poly::zero();
        let mut u = PolyVec::zero();
        let mut u_ntt = PolyVec::zero();
        let mut v = Poly::zero();
        let mut inner = Poly::zero();
        let mu = Poly::from_msg(&msg);
        let mut recovered = [0u8; 32];
        ...
        for i in 0..K {
            u_ntt.polys[i].coeffs.copy_from_slice(&u.polys[i].coeffs);
        }
        u_ntt.ntt();
        s_hat.dot_ntt(&u_ntt, &mut inner);
        inner.inv_ntt();
        v.sub_assign(&inner);
        v.reduce();
        v.to_msg(&mut recovered);
