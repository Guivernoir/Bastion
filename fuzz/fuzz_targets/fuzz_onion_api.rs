...
use crypto_bastion::{
    ...
    LAYER_OVERHEAD, cut, layer_decrypt, layer_encrypt, onion,
};
...
fuzz_target!(|data: &[u8]| {
    ...
    let kem0 = expand_vec(data, KEM_PUBLIC_KEY_SIZE, 0x01);
    let kem1 = expand_vec(data, KEM_PUBLIC_KEY_SIZE, 0x02);
    let kem2 = expand_vec(data, KEM_PUBLIC_KEY_SIZE, 0x03);
    let dsa0 = expand_vec(data, DSA_SECRET_KEY_SIZE, 0x11);
    let dsa1 = expand_vec(data, DSA_SECRET_KEY_SIZE, 0x22);
    let dsa2 = expand_vec(data, DSA_SECRET_KEY_SIZE, 0x33);
    let kem_sk0 = expand_vec(data, KEM_SECRET_KEY_SIZE, 0xA1);
    let kem_sk1 = expand_vec(data, KEM_SECRET_KEY_SIZE, 0xA2);
    let kem_sk2 = expand_vec(data, KEM_SECRET_KEY_SIZE, 0xA3);
    let dsa_pk0 = expand_vec(data, DSA_PUBLIC_KEY_SIZE, 0xB1);
    let dsa_pk1 = expand_vec(data, DSA_PUBLIC_KEY_SIZE, 0xB2);
    let dsa_pk2 = expand_vec(data, DSA_PUBLIC_KEY_SIZE, 0xB3);
    ...
    let mut layer_out = vec![0u8; msg.len() + (3 * LAYER_OVERHEAD)];
    let mut layer_peel_out = vec![0u8; msg.len() + (3 * LAYER_OVERHEAD)];
    ...
    let mut cut_out = vec![0u8; msg.len() + (2 * LAYER_OVERHEAD)];
